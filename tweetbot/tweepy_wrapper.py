...
class TweepyWrapper:
    ...
    def __init__(self,
        ...
        self.__logger = logging.getLogger('TweepyWrapper')
    ...
    def is_follower(self, screen_name: str) -> bool:
        """
        determines if a user name is a follower of the logged in account or not
        ...
        if self.__api_version is TwitterAPIVersion.V1:
            if self.__twitter_api is None:
                raise RuntimeError('Not connected to the twitter API')
            response = self.__twitter_api.lookup_friendships(screen_name=[screen_name])
            if len(response) > 0:
                if response[0].is_followed_by:
                    self.__logger.info(f'{screen_name} is a follower')
                    return True

                self.__logger.info(f'{screen_name} is a not follower')
                return False

            self.__logger.info(f'{screen_name} is not a valid twitter screen handle')
            return False
        elif self.__api_version is TwitterAPIVersion.V2:
            raise NotImplementedError('Not implemented on V2 wrapper yet')
        else:
            raise RuntimeError(f'Unhandled Twitter API version {self.__api_version.name}')
